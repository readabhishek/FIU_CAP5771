...
class TwitterClient(object):
    ...
    def __init__(self):
        ...
        tw_access_token = 'xxxxxxxxxxxxxxxxxxxxx'
        tw_access_token_secret = 'xxxxxxxxxxxxxxxxxxx'
        tw_consumer_key = 'xxxxxxxxxxxxxxxxxx'
        tw_consumer_secret = 'xxxxxxxxxxxxxxxxx'
...
def load_JSON_file(filename, entity):
    query = []; data = json.load(open(filename))
...
def get_tweets_online(twitter_api, query, title):
    positive_tweets, negative_tweets, neutral_tweets = [], [], []

    for key in query:
        tweets_polarity = [0, 1, 2]
        tweets = twitter_api.get_online_tweets(query=key, count=30)
    ...
    print("Neutral tweets percentage: {} %".format(100 * (len(tot_tweets) - len(negative_tweets) - len(positive_tweets)) / len(tot_tweets)))
